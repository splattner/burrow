...
func NewCommand(ctx context.Context, v *viper.Viper) *cobra.Command {
  ...
  BURROW_JWT_HMAC_SECRET=dev-secret BURROW_SERVER_ADDR=:8080 BURROW_BRIDGE_HOST=0.0.0.0 burrow server`,
	...
	flags.String("api-token", "", "Shared bearer token required to call /api/clients/* endpoints (empty = unauthenticated)")
	flags.Bool("enable-client-api", false, "Expose /api/clients/* HTTP endpoints for bridge address discovery")
